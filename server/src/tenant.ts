...
export class TenantDO extends DurableObject<Env> {
  ...
  private async handleAuthenticated(request: Request, url: URL, principal: Principal): Promise<Response> {
    ...
    if (request.method === "GET" && pathname === "/audit") return this.handleAudit(url, principal);
    ...
    this.ctx.storage.transactionSync(() => {
      ...
      for (const key of keys) {
        ...
        if (status === Status.Done) {
          results.push({ key, ticket_id: ticketId, result: "already_done", seq: currentSeq });
          continue;
        }
        if (status === Status.Canceled) {
          results.push({ key, ticket_id: ticketId, result: "canceled", seq: currentSeq });
          continue;
        }
        ...
        const outcome = this.apply(mutation);
        if ("reason" in outcome) throw new Error(`GitHub mutation rejected: ${outcome.reason}`);
        this.audit(outcome.seq, "ticket.update", null, "webhook", "ticket", ticketId, {
          mutation_id: mutation.mutation_id,
          source: "github",
          delivery_id: delivery,
        });
        results.push({ key, ticket_id: ticketId, result: "closed", seq: outcome.seq });
      }
      this.sql.exec(
  ...
  private async handleUpgradeCreation(request: Request, principal: Principal): Promise<Response> {
    ...
    if (live && body.replace !== true) return jsonError(409, "upgrade_already_pending");
  ...
  private async handleTokenUpgrade(request: Request, principal: Principal): Promise<Response> {
    if (!may(principal, "token.self.upgrade")) return jsonError(403, "forbidden");
    const body = await requestObject(request);
    if (!body) return jsonError(400, "invalid_json");
    const enrollment = await this.verifyEnrollment(body.credential ?? body.upgrade_code, "upgrade");
    if (enrollment instanceof Response) return enrollment;
    if (enrollment.member_id !== principal.memberId) return jsonError(401, "invalid_enrollment");
    if (enrollment.intended_access === null) return jsonError(409, "invalid_enrollment_state");
  ...
  private async handleMemberRole(request: Request, principal: Principal): Promise<Response> {
    if (!may(principal, "member.change_role")) return jsonError(403, "forbidden");
    const body = await requestObject(request);
    if (!body) return jsonError(400, "invalid_json");
    const email = invalidEmail(body.email);
  ...
  private handleTokens(url: URL, principal: Principal): Response {
    const all = url.searchParams.get("all") === "1";
    if (all && !may(principal, "token.other.list")) return jsonError(403, "forbidden");
    if (!all && !may(principal, "token.self.list")) return jsonError(403, "forbidden");
