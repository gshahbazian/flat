...
3. Edit a ticket's `title`, `status`, `priority`, `assignee`, `labels`, and
   description. Identity, timestamps, and rendered comments stay read-only.
4. Create a ticket with `flat new TITLE --project KEY`, including existing
   labels with repeatable `--label` flags.
...
7. Use `flat sync --merge` and resolve conflict markers when local and server
   edits overlap.
8. Manage tenant labels with `flat label` and search accepted label state with
   `label:` qualifiers.
9. Delete a server ticket with `flat delete KEY` only on an explicit request
